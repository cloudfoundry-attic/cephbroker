...
var Controller = Describe("Controller", func() {
	...
	Context(".Create", func() {
		It("should be able to create mount", func() {
			resp := subject.Create(logger, voldriver.CreateRequest{
				Name: "InstanceID",
			})
	...
	Context(".Remove", func() {
		It("should be able to remove mount", func() {
			resp := subject.Remove(logger, voldriver.RemoveRequest{Name: "InstanceId"})
			Expect(resp.Err).To(Equal(""))
		})
	})
	Context(".Bind", func() {
		It("should be able to bind", func() {
			resp := subject.Bind(logger, "InstanceId")
			Expect(resp.Err).To(Equal(""))
